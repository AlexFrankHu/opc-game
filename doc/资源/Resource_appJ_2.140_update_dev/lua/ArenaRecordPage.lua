...
function ArenaRecordItem.onRefreshItemView( container )
	...
	local mail = MailInfo.mailAreanAll[index]
	local itemInfo = {
		[1] = "", 	-- name
		[2] = 0,	-- id
		[3] = 0,	-- level
...
function ArenaRecordPageBase:getPageInfo( container )
	if MailInfo.mailAreanAll ~= nil and #MailInfo.mailAreanAll ~= 0 then
		self:rebuildAllItem(container)
	end
	
	local msg = Mail_pb.OPMailInfo()

	if MailInfo.lastMail ~= nil and MailInfo.lastMail.id ~= nil then
...
function ArenaRecordPageBase:buildItem( container )
	if (MailInfo.mailAreanAll == nil) then
		return;
	end

	local maxSize = table.maxn(MailInfo.mailAreanAll);
