...
--]]
----------------------------------------------------------------------------------
RegisterLuaPage('GuildSetAnnouncementPage')
RegisterLuaPage('GuildSetRestrictionPage')
...
function ManageItem.onRefreshItemView(container)
	...
	if 1 == info.itemType then
		...
		local joinLimit = allianceInfo.commonInfo and allianceInfo.commonInfo.battleLimit or 0
	...
	elseif 2 == info.itemType then
		...
		local announcement = allianceInfo.commonInfo and allianceInfo.commonInfo.annoucement or ''
		-- 如果公告太长，取前15个字
...
function ManageItem.setJoinCondition(container)
	-- not the leader
	if MyAllianceInfo.myInfo and MyAllianceInfo.myInfo.postion ~= PositionType.Leader then
