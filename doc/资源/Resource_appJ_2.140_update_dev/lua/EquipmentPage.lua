...
local option = {
	ccbiFile = isIpad and "EquipmentPage_ipad.ccbi" or "EquipmentPage.ccbi",
	handlerMap = {
...
function EquipPageBase:showPlayerInfo(container)
	...
	NodeHelper:setSpriteImage(container, {
		mPerson 	= RoleManager:getPosterById(roleId),
		mOccupation = RoleManager:getOccupationIconById(UserInfo.roleInfo.itemId)
	});
		
	--Test Spine
	if not GameConfig.ShowSpineAvatar then return; end
	local spineContainer = container:getVarNode("mSpineNode");
