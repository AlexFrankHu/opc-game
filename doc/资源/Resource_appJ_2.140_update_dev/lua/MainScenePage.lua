...
function MainScenePageInfo.onFunction(eventName,container)
	...
	elseif eventName == "onAdvertisement" then	
...
function MainScenePageInfo.onEnter(container)
	AppstoreHandle:handleActivityToAds( container )
...
function MainScenePageInfo.onGameMessage(container)
	...
	elseif typeId == MSG_MAINFRAME_REFRESH then
		...
		if string.sub(pageName,1,13) == "MainScenePage" then
            local extraParam = string.sub(pageName,15,-1)
...
function MainScenePageInfo.showPlayerInfo(container)
	...
	-- 开服活动，显示当前登录第几天对应的活动
	if MainScenePageInfo.loginDay>0 and MainScenePageInfo.loginDay<9 then
