...
local EliteMapStyle = VaribleManager:getInstance():getSetting("EliteMapStyle") or 0
EliteMapStyle = tonumber(EliteMapStyle)
if EliteMapStyle==1 then
    function EliteMapManager:onRecieveInfoSync(msg)
        EliteMapManager.passedIdMap = {max = 0};
	    for i=1,#msg.eliteMapInfos do
		    local oneInfo = msg.eliteMapInfos[i]
            if  EliteMapManager.passedIdMap.max <= oneInfo.mapId then
                EliteMapManager.passedIdMap.max = oneInfo.mapId
            end
    ...
    function EliteMapManager:getPassedMapIdByLevel(level)
        --UserInfo.stateInfo.passEliteMapId
	    if EliteMapManager.passedIdMap ~=nil and EliteMapManager.passedIdMap.max~=nil  then
		    return EliteMapManager.passedIdMap.max
	    else
		    return 0
	    end
    end
...
if EliteMapStyle==1 then
    ...
    function EliteMapManager:getNextFightLevelIndex()
	    local passId = EliteMapManager:getPassedMapIdByLevel(0)
        if passId~= 0 then
            local nextId = self:getNextMapId(passId)
            if nextId > passId and self:getLevelIndexById(nextId)~="" then
                return tonumber(self:getLevelIndexById(nextId))
            end
        end
        return 1
    end
