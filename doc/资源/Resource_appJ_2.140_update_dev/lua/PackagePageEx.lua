...
RegisterLuaPage(filterPage);
RegisterLuaPage(batchSellPage);
...
function PackagePageBase:onExit(container)
	...
	container:registerMessage(MSG_MAINFRAME_REFRESH);
    NodeHelper:clearMultiColumnScrollView(container,PageInfo.pageType)
...
function PackagePageBase:clearAllItem(container)    
    container.mScrollView:getContainer():stopAllActions()
    thisScrollViewOffset = NodeHelper:clearMultiColumnScrollView(container,PageInfo.pageType)
...
--缁ф壙姝ょ被鐨勬椿鍔ㄥ鏋滃悓鏃跺紑锛屾秷鎭洃鍚笉鑳藉悓鏃跺瓨鍦?閫氳繃tag鏉ュ尯鍒?
function PackagePageBase:onReceiveMessage(container)
	...
	elseif typeId == MSG_MAINFRAME_REFRESH then
        ...
        local strLen = string.len(thisPageName)
		if string.sub(pageName,1,strLen) == thisPageName then
            local extraParam = string.sub(pageName,strLen+2,-1)
			self:refreshPage(container);
