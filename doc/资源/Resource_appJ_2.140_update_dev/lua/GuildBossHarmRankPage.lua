...
function GuildBossHarmBase:getMyRank(container)
	for i=1, #harmRankList do
		if harmRankList[i].id == MyAllianceInfo.myInfo.id then
...
function GuildBossHarmBase:getHarmRank(container)
	local msg = alliance.HPAllianceHarmSortC()
	local pb = msg:SerializeToString()
	container:sendPakcet(hp.ALLIANCE_HARMSORT_C, pb, #pb, true)
end
