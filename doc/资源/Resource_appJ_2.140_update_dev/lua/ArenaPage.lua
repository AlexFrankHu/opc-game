...
require "HP_pb"
RegisterLuaPage("LeaveMessageDetailPage")
...
function ArenaItem.onRefreshItemView(container)
	...
	if itemInfo:HasField("allianceName") and itemInfo:HasField("allianceId") then
		lb2Str.mGuildName      = itemInfo.allianceName.."(ID "..itemInfo.allianceId..")" 
    else
        lb2Str.mGuildName      = common:getLanguageString("@NoAlliance")
	...
	NodeHelper:setSpriteImage(container, {mProfession = roleConfig[itemInfo.prof%100].proIcon});
	
	local nameNode = container:getVarLabelTTF("mArenaName")

	-- 称号
	nameNode:setString( itemInfo.name .. strPlayType)
...
function ArenaPageBase:initPage( container )
    ...
    NodeHelper:setNodesVisible(container,{mRecordRedPoint = false})
end
