...
function CampMainPageBase:onReceivePacket(container)
	...
	if opcode == HP.CAMPWAR_STATE_SYNC_S or opcode == HP.ROLE_CARRY_SKILL_S then
		--先删除原有timer，重新创建
...
function CampMainPageBase:onReceiveMessage(container)
	...
	if typeId == MSG_MAINFRAME_REFRESH then
		...
		local pageNameLength = string.len(thisPageName)
		if string.sub(pageName,1,pageNameLength) == thisPageName then
		    local extraParam = string.sub(pageName,pageNameLength+2,-1)
