...
--]]
----------------------------------------------------------------------------------
RegisterLuaPage('GuildMemberPopupPage')
...
GuildMembersVar = {
	setMemberList = function (memberInfo)
...
function GuildMemberBase:onEnter(container)
    ...
    AppstoreHandle:handleGuild( container )
	myInfo = MyAllianceInfo.myInfo

	self:registerPackets(container)
	NodeHelper:initScrollView(container, 'mContent', 10)
	self:refreshPage(container)
	self:getMemberList(container)
end
...
function GuildMemberBase:onQuitAlliance(container)
	-- the leader
	if MyAllianceInfo.myInfo 
		and MyAllianceInfo.myInfo.postion == PositionType.Leader 
		and allianceInfo.commonInfo
		and allianceInfo.commonInfo.currentPop > 1 then
...
function GuildMemberBase:doQuitAlliance(container)
	if MyAllianceInfo.myInfo == nil then 
...
function MemberItem.showMemberInfo(container)
	...
	if memberList then 
		...
		if info then
			...
			local myPosition = MyAllianceInfo.myInfo.postion
...
function GuildMemberBase:buildItem(container)
	NodeHelper:buildScrollView(container, #memberList, MemberItem.ccbiFile, MemberItem.onFunction);
end
...
-- alliance members
function GuildMemberBase:getMemberList(container)
	local msg = alliance.HPAllianceMemberC()
	local pb = msg:SerializeToString()
	container:sendPakcet(hp.ALLIANCE_MEMBER_C, pb, #pb, true)
end
