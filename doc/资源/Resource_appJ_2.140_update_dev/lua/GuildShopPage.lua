...
function GuildShopBase:refreshPage(container)
	...
	if MyAllianceInfo.myInfo.contribution then
		lb2Str.mYourContribution = MyAllianceInfo.myInfo.contribution
	end
...
function ShopItem.onRefreshItemView(container)
	...
	for i=1, 3 do
		...
		if subItemIndex > #shopList then return end

		local shopInfo = shopList[subItemIndex]
...
function GuildShopBase:buildItem(container)
	local needItemCount = math.ceil(#shopList / 3)
...
function GuildShopBase:getShopList(container)
	local msg = alliance.HPAllianceShopC()
	local pb = msg:SerializeToString()
	container:sendPakcet(hp.ALLIANCE_SHOP_C, pb, #pb, true)
end
