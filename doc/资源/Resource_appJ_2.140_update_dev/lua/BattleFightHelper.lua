...
function BattleFightHelper:AddMingXiangSkillBattleLog(container,action)
	if FightChar[action.active.pos] == nil then return end
	...
	local content = common:fill(freeTypeStr,action.round,FightChar[action.active.pos].name,action.active.effect.effectValue)	
...
function BattleFightHelper:AddHealSkillBattleLog(container,action)
	...
	for i=1,#action.passive do
		...
		if FightChar[passivePos] == nil then return end
		local content = common:fill(freeTypeStr,action.round,FightChar[action.active.pos].name,
		FightChar[passivePos].name,passive.effect.effectValue)
...
function BattleFightHelper:AddSelfBuffSkillBattleLog(container,action)
	...
	local content = common:fill(freeTypeStr,action.round,FightChar[action.active.pos].name,skillName)
...
function BattleFightHelper:AddSkipBattleBattleLog(container,action)
	...
	local content = common:fill(freeTypeStr,action.round,FightChar[action.active.pos].name,buffStr)	
	BattleFightHelper:AddCotentIntoSVByString(container,content)
end
...
function BattleFightHelper:AddAvoidBuffLog(container,action)
	if FightChar[action.active.pos] == nil then return end	
	if FightChar[action.active.pos] == nil then return end	
	if #action.passive == 0 or action.passive[1].pos == nil or FightChar[action.passive[1].pos]==nil then return end
	...
	if isOdd then
		content = common:fill(freeTypeStr,action.round,avoidReason,FightChar[action.active.pos].name,buffStr)	
	else
		content = common:fill(freeTypeStr,action.round,buffStr,FightChar[action.passive[1].pos].name,avoidReason)	
	end
...
function BattleFightHelper:AddBuffForAllSelfBattleLog(container,action)
	...
	for i=1,#action.passive do
		...
		local passivePos = passive.pos
		if FightChar[passivePos] == nil then return end	
		local content = common:fill(FightLogConfig["UseBuffForOther"].content,action.round,FightChar[action.active.pos].name,
		FightChar[passivePos].name,skillName)
		...
		if FightLogConfig~=nil  then
			...
			if passives~=nil then
				for i=1,#passives do
					...
					if FightChar[activePerson]~=nil and  FightChar[passivePerson]~=nil then
						...
						if action:HasField("skillId") then
							...
							local content = common:fill(skillFreeTypeStr,action.round,FightChar[activePerson].name,
							skillName,FightChar[passivePerson].name,passive.effect.effectValue)
							...
							if skillId == 13 then
								local skillId = action.skillId
								local content = common:fill(FightLogConfig["XiXueSkill"].content,action.round,FightChar[action.active.pos].name,action.active.effect.effectValue)
								BattleFightHelper:AddCotentIntoSVByString(container,content)
							end
						else
							--普通攻击
							...
							local content = common:fill(attackFreeTypeStr,action.round,FightChar[activePerson].name,
							FightChar[passivePerson].name,passive.effect.effectValue)
							if passive.effect.effectType == Battle_pb.EFFECT_CRITS then
								content = content..FightLogConfig["EFFECT_CRITS"].content
							end
							if passive.effect.effectType == Battle_pb.EFFECT_MISS then
