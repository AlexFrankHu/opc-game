...
function BattlePageInfo.onEnter(container)
	...
	--获取历史聊天的Layer，注册事件响应
	mSpeakLayer = container:getVarNode("mSpeakHistoryLayer");
    mSpeakLayer = tolua.cast(mSpeakLayer,"CCLayer")
...
function BattlePageInfo.onGameMessage(container)
	...
	if container:getMessage():getTypeId() == MSG_MAINFRAME_REFRESH then	
		...
		if string.sub(pageName,1,10)=="BattlePage" then
			local extraParam = string.sub(pageName,12,-1)
