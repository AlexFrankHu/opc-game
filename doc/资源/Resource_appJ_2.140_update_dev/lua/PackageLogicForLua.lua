	...
	if msg~=nil then
		...
		if msg:HasField("rewards") then
			...
			if rewards:HasField("contribution") then
				if MyAllianceInfo and MyAllianceInfo.myInfo
					and MyAllianceInfo.myInfo.contribution then
					MyAllianceInfo.myInfo.contribution = rewards.contribution
				end
			end
...
--处理error code 的飘字情况，现在error code 其实充当了Status code的角色 by zhenhui 2014/8/23
...
function PackageLogicForLua.onReceiveOfflineMessageBox(eventName,handler)
	if eventName == "luaReceivePacket" then
		...
		msg:ParseFromString(msgbuff)
		if msg~=nil then
			hasNewChatComing = true -- 离线新消息世界聊天显示红点
...
function PackageLogicForLua.onReceiveGuildBossVitality(eventName, handler)
	if eventName == "luaReceivePacket" then
		...
		msg:ParseFromString(msgbuff);
		allianceInfo.commonInfo.curBossVitality = msg.curBossVitality
		allianceInfo.commonInfo.openBossVitality = msg.openBossVitality
...
--用于function失效后，校验并重注册使用。  by zhenhui 2014/9/3
